...
function run_tests {
  ...
  if [ $debug -eq 1 ]; then
    if [ "$testropts" = "" ] && [ "$testrargs" = "" ]; then
      ...
      # provided.
      testrargs="discover ./miper/tests"
    fi
  ...
  if [ setup.cfg -nt miper.egg-info/entry_points.txt ]
  then
  ...
  if [ $coverage -eq 1 ]; then
    ...
    ${wrapper} coverage html --include='miper/*' --omit='miper/openstack/common/*' -d covhtml -i
  fi
...
function run_pep8 {
  ...
  ${wrapper} tools/check_exec.py miper || exit 1
}
