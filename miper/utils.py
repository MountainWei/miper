...
synchronized = lockutils.synchronized_with_prefix('miper-')
...
def find_config(config_path):
    ...
    :raises: `miper.exception.ConfigNotFound`

    """
    possible_locations = [
        config_path,
        os.path.join(CONF.state_path, "etc", "miper", config_path),
        os.path.join(CONF.state_path, "etc", config_path),
        os.path.join(CONF.state_path, config_path),
        "/etc/miper/%s" % config_path,
    ]
...
def miperdir():
    import miper
    return os.path.abspath(miper.__file__).split('miper/__init__.py')[0]
...
def monkey_patch():
    ...
    Example: 'miper.api.ec2.cloud:' \
     miper.openstack.common.notifier.api.notify_decorator'

    Parameters of the decorator is as follows.
    (See miper.openstack.common.notifier.api.notify_decorator)
...
def get_root_helper():
    return 'sudo miper-rootwrap %s' % CONF.rootwrap_config
