...
"""Miper common internal object model"""
...
class MiperObjectRegistry(base.VersionedObjectRegistry):
    def registration_hook(self, cls, index):
        setattr(objects, cls.obj_name(), cls)


@MiperObjectRegistry.register
class MiperObject(base.VersionedObject):
    # NOTE(thangp): OBJ_PROJECT_NAMESPACE needs to be set so that nova,
    # miper, and other objects can exist on the same bus and be distinguished
    # from one another.
    OBJ_PROJECT_NAMESPACE = 'miper'

    # NOTE(thangp): As more objects are added to miper, each object should
    # have a custom map of version compatibility.  This just anchors the base
    # version compatibility.
    VERSION_COMPATIBILITY = {'7.0.0': '1.0'}

    def miper_obj_get_changes(self):
...
class MiperObjectDictCompat(base.VersionedObjectDictCompat):
    ...
    def get(self, key, value=base._NotSpecifiedSentinel):
        ...
        if key not in self.obj_fields:
            ...
            # rely on it.
            LOG.debug('Miper object %(object_name)s has no '
...
class MiperPersistentObject(object):
...
class MiperComparableObject(base.ComparableVersionedObject):
    def __eq__(self, obj):
...
class MiperObjectSerializer(base.VersionedObjectSerializer):
    OBJ_BASE_CLASS = MiperObject
