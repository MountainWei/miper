...
from miper.objects import base
from miper.tests.unit import objects as test_objects


@base.MiperObjectRmipertry.register
class TestObject(base.MiperObject):
    fields = {
...
class TestMiperObject(test_objects.BaseObjectsTestCase):
    """Tests methods from MiperObject."""

    def setUp(self):
        super(TestMiperObject, self).setUp()
        self.obj = TestObject(
            scheduled_at=None,
            uuid=uuid.uuid4(),
            text='text')
    ...
    def test_miper_obj_get_changes_no_changes(self):
        self.assertDictEqual({}, self.obj.miper_obj_get_changes())

    def test_miper_obj_get_changes_other_changes(self):
        self.obj.text = 'text2'
        self.assertDictEqual({'text': 'text2'},
                             self.obj.miper_obj_get_changes())

    def test_miper_obj_get_changes_datetime_no_tz(self):
        now = datetime.datetime.utcnow()
        self.obj.scheduled_at = now
        self.assertDictEqual({'scheduled_at': now},
                             self.obj.miper_obj_get_changes())

    def test_miper_obj_get_changes_datetime_tz_utc(self):
    ...
    def test_miper_obj_get_changes_datetime_tz_non_utc_positive(self):
    ...
    def test_miper_obj_get_changes_datetime_tz_non_utc_negative(self):
...
class TestMiperComparableObject(test_objects.BaseObjectsTestCase):
