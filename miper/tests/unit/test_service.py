...
test_service_opts = [
    cfg.StrOpt("fake_manager",
               default="miper.tests.unit.test_service.FakeManager",
...
class ServiceManagerTestCase(test.TestCase):
    ...
    def test_message_gets_to_manager(self):
        serv = service.Service('test',
                               'test',
                               'test',
                               'miper.tests.unit.test_service.FakeManager')
        serv.start()
...
class TestWSGIService(test.TestCase):
    ...
    @mock.patch('miper.wsgi.eventlet_server.Server')
    def test_workers_set_default(self, wsgi_server):
        test_service = service.WSGIService("osapi_miper")
    ...
    def test_workers_set_good_user_setting(self, wsgi_server):
        self.override_config('osapi_miper_workers', 8)
        test_service = service.WSGIService("osapi_miper")
        self.assertEqual(8, test_service.workers)

    @mock.patch('miper.wsgi.eventlet_server.Server')
    def test_workers_set_zero_user_setting(self, wsgi_server):
        self.override_config('osapi_miper_workers', 0)
        test_service = service.WSGIService("osapi_miper")
    ...
    def test_workers_set_negative_user_setting(self, wsgi_server):
        self.override_config('osapi_miper_workers', -1)
...
class OSCompatibilityTestCase(test.TestCase):
    def _test_service_launcher(self, fake_os):
        # Note(lpetrut): The miper-mipermanager service needs to be spawned
        ...
        with mock.patch('os.name', fake_os):
            with mock.patch('miper.service.process_launcher',
