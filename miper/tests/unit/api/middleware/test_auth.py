...
import miper.api.middleware.auth
from miper import test


class TestMiperKeystoneContextMiddleware(test.TestCase):

    def setUp(self):
        super(TestMiperKeystoneContextMiddleware, self).setUp()

        @webob.dec.wsgify()
        def fake_app(req):
            self.context = req.environ['miper.context']
            return webob.Response()

        self.context = None
        self.middleware = (miper.api.middleware.auth
                           .MiperKeystoneContext(fake_app))
