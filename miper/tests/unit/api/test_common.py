...
class LimiterTest(test.TestCase):
    """Unit tests for the `miper.api.common.limited` method.
...
class PaginationParamsTest(test.TestCase):
    """Unit tests for `miper.api.common.get_pagination_params` method.
...
class LinkPrefixTest(test.TestCase):
    def test_update_link_prefix(self):
        ...
        result = vb._update_link_prefix("http://192.168.0.243:24/",
                                        "http://127.0.0.1/miper")
        self.assertEqual("http://127.0.0.1/miper", result)
...
class RequestUrlTest(test.TestCase):
    def test_get_request_url_no_forward(self):
        app_url = 'http://127.0.0.1/v1;param?key=value#frag'
        request = type('', (), {
    ...
    def test_get_request_url_forward(self):
        request = type('', (), {
            'application_url': 'http://127.0.0.1/v1;param?key=value#frag',
            'headers': {'X-Forwarded-Host': '192.168.0.243:24'}
        })
        result = common.get_request_url(request)
        self.assertEqual('http://192.168.0.243:24/v1;param?key=value#frag',
                         result)
