...
from miper.api import extensions
from miper.api.v1 import router
from miper.api import xmlutil
from miper import test


NS = "{https://www.openstack.org/mediawiki/miper/api/v1.0}"
...
class ExtensionTestCase(test.TestCase):
    def setUp(self):
        ...
        ext_list = CONF.osapi_miper_extension[:]
        fox = ('miper.tests.unit.api.extensions.foxinsocks.Foxinsocks')
        if fox not in ext_list:
            ext_list.append(fox)
            self.flags(osapi_miper_extension=ext_list)
