...
from miper.hacking import checks
from miper import test


@ddt.ddt
class HackingTestCase(test.TestCase):
    """This class tests the hacking checks in miper.hacking.checks

    This class ensures that Miper's hacking checks are working by passing
    ...
    def test_no_translate_debug_logs(self):
        self.assertEqual(1, len(list(checks.no_translate_debug_logs(
            "LOG.debug(_('foo'))", "miper/scheduler/foo.py"))))

        self.assertEqual(0, len(list(checks.no_translate_debug_logs(
            "LOG.debug('foo')", "miper/scheduler/foo.py"))))

        self.assertEqual(0, len(list(checks.no_translate_debug_logs(
            "LOG.info(_('foo'))", "miper/scheduler/foo.py"))))

    def test_check_explicit_underscore_import(self):
        self.assertEqual(1, len(list(checks.check_explicit_underscore_import(
            "LOG.info(_('My info message'))",
            "miper.tests.unit/other_files.py"))))
        self.assertEqual(1, len(list(checks.check_explicit_underscore_import(
            "msg = _('My message')",
            "miper.tests.unit/other_files.py"))))
        ...
        self.assertEqual(0, len(list(checks.check_explicit_underscore_import(
            ...
            "miper.tests.unit/other_files.py"))))
        self.assertEqual(0, len(list(checks.check_explicit_underscore_import(
            "from miper.i18n import _, _LW",
            "miper.tests.unit/other_files2.py"))))
        self.assertEqual(0, len(list(checks.check_explicit_underscore_import(
            "msg = _('My message')",
            "miper.tests.unit/other_files2.py"))))
        self.assertEqual(0, len(list(checks.check_explicit_underscore_import(
            "_ = translations.ugettext",
            "miper.tests.unit/other_files3.py"))))
        self.assertEqual(0, len(list(checks.check_explicit_underscore_import(
            "msg = _('My message')",
            "miper.tests.unit/other_files3.py"))))
        ...
        self.assertEqual(0, len(list(checks.check_explicit_underscore_import(
            ...
            "miper.tests.unit/other_files4.py"))))
    ...
    def test_logging_format_args(self):
        ...
        code = """
               ...
               LOG.debug("Miper %s caught fire and is at %d degrees C and"
                         " climbing.", 'miper1', 500)
               """
        ...
        code = """
               import logging
               LOG = logging.getLogger()
               LOG.{0}("Miper %s caught fire and is at %d degrees C and "
                      "climbing.", ('miper1', 500))
               """
        ...
        code = """
               import logging
               LOG = logging.getLogger()
               LOG.log(logging.DEBUG, "Miper %s caught fire and is at %d"
                       " degrees C and climbing.", ('miper1', 500))
               """
    ...
    def test_validate_log_translations(self):
        ...
        self.assertEqual(0, len(list(checks.validate_log_translations(
            "LOG.info('foo')", "miper/tests/foo.py"))))
        self.assertEqual(0, len(list(checks.validate_log_translations(
    ...
    def test_no_print_statements(self):
        ...
        self.assertEqual(1, len(list(checks.check_no_print_statements(
            ...
            "miper/file.py", False))))
        self.assertEqual(0, len(list(checks.check_no_print_statements(
            "print('My print statement in miper/cmd, which is ok.')",
            "miper/cmd/file.py", False))))
        self.assertEqual(0, len(list(checks.check_no_print_statements(
            "print('My print statement that I just must have.')",
            "miper.tests.unit/file.py", True))))
        self.assertEqual(1, len(list(checks.check_no_print_statements(
            "print ('My print with space')",
            "miper/miper/anotherFile.py", False))))
    ...
    @ddt.unpack
    @ddt.data(
        (1, 'LOG.info', "miper/tests/unit/fake.py", False),
        (1, 'LOG.warning', "miper/tests/fake.py", False),
        (1, 'LOG.error', "miper/tests/fake.py", False),
        (1, 'LOG.exception', "miper/tests/fake.py", False),
        (1, 'LOG.debug', "miper/tests/fake.py", False),
        (0, 'LOG.info.assert_called_once_with', "miper/tests/fake.py", False),
        (0, 'some.LOG.error.call', "miper/tests/fake.py", False),
        (0, 'LOG.warning', "miper/tests/unit/fake.py", True),
        (0, 'LOG.warning', "miper/tests/unit/integrated/fake.py", False))
