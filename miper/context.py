...
"""RequestContext: context for requests that persist through all of miper."""
...
from miper import policy

context_opts = [
    cfg.StrOpt('miper_internal_tenant_project_id',
               default=None,
               help='ID of the project which will be used as the Miper '
                    'internal tenant.'),
    cfg.StrOpt('miper_internal_tenant_user_id',
               default=None,
               help='ID of the user to be used in mipermanager operations as the '
                    'Miper internal tenant.'),
]
...
class RequestContext(context.RequestContext):
    ...
    # tenant/user whereas the Miper version uses project_id/user_id.
    # NOTE(adrienverge): The Miper version of RequestContext now uses
    # tenant/user internally, so it is compatible with context-aware code from
    # openstack/common. We still need this shim for the rest of Miper's
    # code.
    @property
    def project_id(self):
        return self.tenant
...
def get_internal_tenant_context():
    """Build and return the Miper internal tenant context object

    This request context will only work for internal Miper operations. It will
    not be able to make requests to remote services. To do so it will need to
    use the keystone client to get an auth_token.
    """
    project_id = CONF.miper_internal_tenant_project_id
    user_id = CONF.miper_internal_tenant_user_id
