...
Guidelines for writing new hacking checks
 ...
 - Use only for Miper specific tests. OpenStack general tests
 ...
 - Add test cases for each new rule to
   miper/tests/test_hacking.py

"""

# NOTE(thangp): Ignore N323 pep8 error caused by importing miper objects
UNDERSCORE_IMPORT_FILES = ['./miper/objects/__init__.py']
...
class CheckLoggingFormatArgs(BaseASTChecker):
    ...
    LOG.debug("Miper %s caught fire and is at %d degrees C and climbing.",
              ('miper1', 500))
...
def no_test_log(logical_line, filename, noqa):
    if "miper/tests" not in filename or noqa:
        return
    # Skip the "integrated" tests for now
    if "miper/tests/unit/integrated" in filename:
        return
